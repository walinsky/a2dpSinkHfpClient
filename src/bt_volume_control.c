...
/**
 * @brief Internal: Apply HFP speaker volume.
 */
static esp_err_t apply_hfp_speaker_volume(uint8_t volume)
{
    esp_err_t ret = a2dpSinkHfpHf_set_hfp_speaker_volume(volume);
    if (ret != ESP_OK) {
...
/**
 * @brief Internal: Apply HFP microphone volume.
 */
static esp_err_t apply_hfp_mic_volume(uint8_t volume)
{
    esp_err_t ret = a2dpSinkHfpHf_set_hfp_mic_volume(volume);
    if (ret != ESP_OK) {
...
/**
 * @brief Internal: Track A2DP volume locally.
 *
 * Note: A2DP sink volume may be controlled by the phone via AVRCP absolute
 * volume. This function only updates local state and callbacks.
 */
...
/**
 * @brief Initialize Bluetooth volume control.
 */
esp_err_t bt_volume_control_init(const bt_volume_config_t *config)
{
    ...
    g_current_a2dp_volume =
        (config->default_a2dp_volume > VOLUME_MAX) ? VOLUME_MAX : config->default_a2dp_volume;
    g_current_hfp_speaker_volume =
        (config->default_hfp_speaker_volume > VOLUME_MAX) ?
        VOLUME_MAX : config->default_hfp_speaker_volume;
    g_current_hfp_mic_volume =
        (config->default_hfp_mic_volume > VOLUME_MAX) ?
...
/**
 * @brief Set volume for specific target.
 */
esp_err_t bt_volume_control_set(bt_volume_target_t target, uint8_t volume)
{
    ...
    case BT_VOLUME_TARGET_A2DP:
        // A2DP volume is usually controlled by phone via AVRCP - track locally
...
/**
 * @brief Get current volume for specific target.
 */
...
/**
 * @brief Increase volume by specified amount.
 */
...
/**
 * @brief Decrease volume by specified amount.
 */
...
/**
 * @brief Mute or unmute specific volume target.
 */
esp_err_t bt_volume_control_mute(bt_volume_target_t target, bool mute)
{
    ...
    case BT_VOLUME_TARGET_HFP_SPEAKER:
        if (mute && !g_hfp_speaker_muted) {
            ...
            ESP_LOGI(TAG, "HFP speaker unmuted (restored to %d)",
                     g_hfp_speaker_volume_before_mute);
        }
        break;
    ...
    case BT_VOLUME_TARGET_HFP_MIC:
        if (mute && !g_hfp_mic_muted) {
            ...
            ESP_LOGI(TAG, "HFP mic unmuted (restored to %d)",
                     g_hfp_mic_volume_before_mute);
        }
        break;
...
/**
 * @brief Check if specific target is currently muted.
 */
...
/**
 * @brief Reset to default volumes.
 */
