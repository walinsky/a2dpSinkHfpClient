...
/* installation for i2s; done in bt_app_i2s */
// static void bt_i2s_driver_install(void);
/* uninstallation for i2s; done in bt_app_i2s */
...
static void volume_set_by_controller(uint8_t volume)
{
    ESP_LOGI(BT_RC_TG_TAG, "Volume is set by remote controller to %d: %"PRIu32"%%", volume, (uint32_t)volume * 100 / 0x7f);
...
{
    bt_app_adjust_volume((uint8_t *)data, len);
